...
PRIMARY_MONITOR_EXECS_CONF = Path.home() / ".config/hypr/hyprland/execs.conf"
...
def _save_monitors_lua_and_reload(monitors: list) -> None:
    ...
    path = resolve_path("monitors", Provider.LUA)
    lines = [caelestia_core.render_lua_call("hl.monitor", [_build_monitor_lua_table(m)]) for m in monitors]
    write_managed_lua_block_and_reload(path, MONITORS_LUA_BLOCK, lines)
...
def _get_primary_monitor() -> str:
    ...
    execs = PRIMARY_MONITOR_EXECS_CONF
    if not execs.exists():
        return ""
...
def _set_primary_monitor(name: str):
    ...
    require_config_capability(ConfigCapability.EXECS, writer_provider=Provider.HYPRLANG)
    execs = PRIMARY_MONITOR_EXECS_CONF
...
class MonitorSettingsPanel(Gtk.Box):
    ...
    def load(self, m: MonitorModel | None):
        ...
        primary_available = load_provider() is Provider.HYPRLANG
        self._primary_row.set_sensitive(primary_available)
        self._primary_row.set_subtitle(
            t("xrandr --primary (for X11 apps and tray)")
            if primary_available
            else t("Primary monitor selection is not yet available for Lua configuration.")
        )
...
class MonitorPage(Gtk.Box):
    ...
    def load_monitors(self):
        ...
        provider = load_provider()
        primary_name = _get_primary_monitor() if provider is Provider.HYPRLANG else ""
    ...
    def _on_apply(self, _btn):
        ...
        for m in self._monitors:
            if provider is Provider.HYPRLANG and m.primary:
