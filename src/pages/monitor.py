...
class MonitorPage(Gtk.ScrolledWindow):
    ...
    def load_monitors(self):
        ...
        try:
            ...
            if not active_mons:
                self.monitor_list_box.append(Gtk.Label(label="Keine aktiven Monitore."))
                return
            ...
            self.monitor_list_box.append(Gtk.Label(label=f"Fehler: {e}"))
...
class MonitorWidget(Adw.PreferencesGroup):
    def __init__(self, monitor_data, saved_settings, all_names, parent_page):
        ...
        self.set_title(f"Monitor: {self.name} ({monitor_data['description']})")
        
        # 1. Auflösung
        res_row = Adw.ActionRow(title="Auflösung & Frequenz")
        ...
        # 2. Anordnung
        arrange_row = Adw.ActionRow(title="Anordnung")
        ...
        self.cur_pos_str = f"Benutzerdefiniert ({monitor_data['x']}x{monitor_data['y']})"
        self.arrange_combo.append(self.cur_pos_id, self.cur_pos_str)

        opts = [("auto", "Automatisch"), ("rechts", "Rechts von"), ("links", "Links von"), 
                ("darueber", "Darüber"), ("darunter", "Darunter"), ("spiegeln", "Spiegeln"), ("deaktivieren", "Deaktivieren")]
        for k, v in opts: self.arrange_combo.append(k, v)
        
        # Standard: Gespeichert > Deaktiviert (falls Monitor inaktiv) > Spiegeln > Aktuelle Pos
